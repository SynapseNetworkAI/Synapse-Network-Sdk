...
- Repo hygiene: retired gateway domains, deprecated brand wording, README language split, prod defaults, public example credential names, string money examples, removed local gateway guidance, and sensitive tracked filenames.
...
- Public examples must use `SYNAPSE_AGENT_KEY`, production defaults, `https://api.synapse-network.ai`, and string money values. Staging may appear only as sandbox/E2E guidance with Arbitrum Sepolia and MockUSDC.
